...
def create_validation_alert(alert_name, cron_frequency, cron_expression, email_list, validation_query, report_table_query):
  ...
  p = util.fetch_data(constants.create_validation_procedure_query.replace('PROCEDURE_NAME', procedure_name))
  ...
  util.create_db_entry('Validation Alert', task_name, procedure_name, alert_name, cron_frequnecy, cron_expression, time_zone, current_user, email_list, validation_query, report_table_query, '', conn)
...
def create_alert(alert_type, alert_name, cron_frequency, cron_expression, email_list, kpi_query=None, validation_query=None, report_table_query=None):
  try:
    ...
    if ~util.validate_cron_expression(cron_expression):
